...
async function runTests() {
	...
	try {
		...
		const scanner = new WorkspaceScanner(testWorkspacePath);
		...
		const packageManager = new PackageManager(scanner);
		...
		allPackages.forEach((pkg) => {
			...
			console.log(`  ${pkg}:`);
			Array.from(versionInfo.versions.entries()).forEach(
				([version, workspaces]) => {
		...
		console.log(
			`🔍 Found ${conflicts.length} packages with version conflicts:`
		);
		conflicts.forEach((conflict) => {
			console.log(`  ${conflict.packageName}:`);
			Array.from(conflict.versions.entries()).forEach(
				([version, workspaces]) => {
					const workspaceNames = workspaces.map((ws) => ws.name).join(', ');
					console.log(`    ${version}: ${workspaceNames}`);
				}
		...
		expectedCommands.forEach((cmd) => {
			const found = commands.find((c: any) => c.command === cmd);
			if (found) {
